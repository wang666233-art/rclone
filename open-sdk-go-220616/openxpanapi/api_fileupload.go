...
func (r ApiPcssuperfile2Request) FileReader(reader io.Reader, name string) ApiPcssuperfile2Request {
	r.fileReader = reader
	r.fileName = name
	return r
}
...
func (a *FileuploadApiService) Pcssuperfile2Execute(r ApiPcssuperfile2Request) (string, *_nethttp.Response, error) {
	...
	if r.fileReader != nil {
		fbs, readErr := _ioutil.ReadAll(r.fileReader)
		if readErr != nil {
			return localVarReturnValue, nil, readErr
		}
		fileLocalVarFileBytes = fbs
		fileLocalVarFileName = r.fileName
		if fileLocalVarFileName == "" {
			fileLocalVarFileName = "chunk"
		}
	} else {
		...
		if fileLocalVarFile != nil {
			fbs, err := _ioutil.ReadAll(fileLocalVarFile)
			if err != nil {
...
func (r ApiXpanfilecreateRequest) Size(size int64) ApiXpanfilecreateRequest {
	r.size = &size
	return r
}
...
// size
func (r ApiXpanfileprecreateRequest) Size(size int64) ApiXpanfileprecreateRequest {
	r.size = &size
	return r
}
